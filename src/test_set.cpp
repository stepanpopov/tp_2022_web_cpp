...
void check_constness() {
    ...
    if (s.size() != 5)
        fail("incorrect size");
    auto first = s.begin();
    tp::set<int>::iterator last = s.end();
    if (*first != -4 || *(--last) != 7)
...
void check_empty() {
    ...
    tp::set<std::string> empty;
    tp::set<std::string> s2{"opa"};
    s2 = empty;
    if (s2.size())
        fail("incorrect size");
    tp::set<std::string> s3(s2);
    if (!s3.empty())
        fail("incorrect empty");
    std::cerr << "ok!\n";
}
...
void check_iterators() {
    ...
    tp::set<std::pair<int, int> > s{{-3, 5}, {5, 5}, {-4, 1}, {-4, 4}, {0, 1}, {3, 0}};
    if (s.begin()->second != 1 || (++s.begin())->first != -4)
        fail("wrong begin()");
    tp::set<std::pair<int, int> >::iterator cur = s.end();
    tp::set<int> small{1};
    tp::set<int>::iterator it;
    it = small.begin();
    if (*it != 1)
...
void check_operator_less() {
    ...
    tp::set<StrangeInt> s{-5, -3, -6, 13, 7, 1000, 963};
...
void check_destructor() {
    ...
    StrangeInt::init();
    {
        tp::set<StrangeInt> s{5, 4, 3, 2, 1, 0};
        if (s.size() != 6)
            fail("wrong size");
    }
    ...
    {
        tp::set<StrangeInt> s{-3, 3, -2, 2, -1, 1};
        tp::set<StrangeInt> s1(s);
        s1.insert(0);
        tp::set<StrangeInt> s2(s1);
        if (s1.find(0) == s1.end())
            fail("wrong find");
        s1 = s;
        if (s1.find(0) != s1.end())
...
void check_erase() {
    ...
    tp::set<std::string> s{"abacaba", "hello", "p"};
    s.erase("miss");
    s.erase("hello");
    if (s.size() != 2)
        fail("Bad size");
